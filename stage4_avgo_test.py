"""
Stage 4 — Live end-to-end pipeline test, AVGO, layer-by-layer.

Usage:
    python stage4_avgo_test.py --layer 1   # fetch + calc_baseline
    python stage4_avgo_test.py --layer 2   # run_pass1_foundation  (LLM)
    python stage4_avgo_test.py --layer 3   # fetch_peer_metrics + run_methodology_math
    python stage4_avgo_test.py --layer 4   # run_pass2_report      (LLM)
    python stage4_avgo_test.py --layer 5   # run_pass3_audit       (LLM) + final summary

METHODOLOGY_VERSION stays v1.  No v1 code is touched.  No flag is flipped.
State is persisted in stage4_state/ between layers.
"""
...
TICKER = "AVGO"
STATE_DIR = Path(__file__).parent / "stage4_state"
...
def layer3() -> None:
    ...
    # Merge into baseline (same logic as run_pipeline in ai.py:1387)
    ...
    print(f"  joint_probs:            bull={jp.get('bull'):.3f}  base={jp.get('base'):.3f}  bear={jp.get('bear'):.3f}")
    print(f"  joint_probs sum:        {sum(jp.values()):.6f}")
...
def layer5() -> None:
    ...
    print(f"\n  AVGO current_price:      ${current_price}")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Stage 4 layer-by-layer pipeline test (AVGO)")
