...
| 15 | Peer-set composition is ungoverned and can flip the verdict | reliability / threshold | Now demonstrated across THREE runs of CLS — three different peer sets, two different verdicts: (1) test fixture (FLEX/JBL/SANM): base_pe 27.23 → WATCH +12.4%; (2) _live_out_CLS.json (adds ANET): base_pe 28.93 → BUY +23.6%; (3) live run 2026-07-04 (FLEX/JBL/SANM, no ANET): base_pe 25.24 → BUY +13.5%. Verdict swings with fetched peer composition — the dominant ungoverned input near the CLS threshold. Same class as NVDA temperature swing and LLM-margin (#14). quality_adj CAP does the flip work in run (2) — ANET trips the 1.30 cap; ties to the quality_adj calibration item. Fix: deterministic peer-selection rule + borderline band so near-threshold names show sensitivity, not a false-confident verdict. |

## Security / operations
| # | Item | Type | Notes |
|---|------|------|-------|
| 16 | Three legacy accounts keep their pre-exposure password hashes | ACCEPTED RISK (2026-07-31) | `users.json` sat in the PUBLIC repo from 2026-04-17 to 2026-07-31 (~3.5 months), so the bcrypt hashes and emails for all 4 accounts must be assumed collected. `mayukhk` was reset; `mayukh151`, `mayukh123`, `mayukh1` were deliberately left as-is — all are the owner's own throwaway/test accounts with throwaway passwords, and there is no third party to notify. Hashes are bcrypt cost-12, so a strong password remains safe; a weak one does not. **Residual risk is password REUSE, not PickR** — if any of those passwords is used elsewhere, the exposure is at that other service. Revisit only if any of these accounts becomes real or is given to someone else. |
| 17 | Emails remain in public git history | ACCEPTED RISK (2026-07-31) | `git rm --cached` removed users.json/reports/tracked_stocks.json from tracking, but the public repo's HISTORY still contains two of the owner's own email addresses. Purging needs `git filter-repo` + a force-push that breaks every existing clone — judged disproportionate for two self-owned addresses. Revisit before onboarding any real third-party user. |
| 18 | Local venv is Python 3.9.6 (EOL) while prod/CI are 3.12 | test-gap | `runtime.txt` and both workflows pin 3.12; the local venv is 3.9.6, EOL since Oct 2025. Tests therefore pass on a version nothing else runs. Code is deliberately version-portable (typing.Optional/List, no `X \| Y`, no match, no tomllib) so this is latent, not active. Rebuild: `python3.12 -m venv venv --clear && ./venv/bin/pip install -r requirements.txt`. See docs/OPERATIONS.md §5. |
