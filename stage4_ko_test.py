"""
Stage 4 — Live end-to-end pipeline test, KO, layer-by-layer.

Usage:
    python stage4_ko_test.py --layer 1   # fetch + calc_baseline
    python stage4_ko_test.py --layer 2   # run_pass1_foundation  (LLM)
    python stage4_ko_test.py --layer 3   # fetch_peer_metrics + run_methodology_math
    python stage4_ko_test.py --layer 4   # run_pass2_report      (LLM)
    python stage4_ko_test.py --layer 5   # run_pass3_audit       (LLM) + final summary

METHODOLOGY_VERSION stays v1.  No v1 code is touched.  No flag is flipped.
State is persisted in stage4_state_ko/ between layers.

KO is a low-growth consumer staples franchise — chosen specifically to exercise
Bug A (bear P/E floor inversion fix) and Bug B (base P/E ratio-discount fix).
"""
...
TICKER = "KO"
STATE_DIR = Path(__file__).parent / "stage4_state_ko"
...
def layer1() -> None:
    ...
    print(f"  company_name:          {baseline.get('company_name')}")
    print(f"  fy_revenue:            {baseline.get('fy_revenue')}")
    print(f"  fy_eps_non_gaap:       {baseline.get('fy_eps_non_gaap')}")
    print(f"  five_yr_eps_growth_est:{baseline.get('five_yr_eps_growth_est')}")
...
def layer3() -> None:
    ...
    ev  = math.get("expected_value")
    cd  = math.get("consensus_divergent")
    cal = math.get("calibration_log", [])
    rec = math.get("recommendation")

    # Raw bull EPS before Step A: parse from calibration_log if Step A fired
    raw_bull_pre_step_a = None
    for entry in cal:
        if entry.startswith("Step A: bull EPS floored $"):
            # format: "Step A: bull EPS floored $X.XX→$Y.YY ..."
            try:
                after_dollar = entry.split("$")[1]
                raw_bull_pre_step_a = float(after_dollar.split("→")[0])
            except Exception:
                raw_bull_pre_step_a = f"(parse failed from: {entry!r})"
            break

    print(f"  scenario_eps.bull:      {se.get('bull')}")
    print(f"  price_target.bull_high: {pt.get('bull_high')}")
    print(f"  scenario_eps.base:      {se.get('base')}")
    print(f"  scenario_eps.bear:      {se.get('bear')}")
    print()
    bull_p = jp.get("bull", 0)
    base_p = jp.get("base", 0)
    bear_p = jp.get("bear", 0)
    print(f"  joint_probs:  bull={bull_p:.3f}  base={base_p:.3f}  bear={bear_p:.3f}  sum={bull_p+base_p+bear_p:.6f}")
    print(f"  expected_value:         {ev}")
    print(f"  consensus_divergent:    {cd}")
    print(f"  recommendation:         {rec}")
    print()
    if raw_bull_pre_step_a is not None:
        print(f"  [Step A fired] raw bull EPS pre-floor: {raw_bull_pre_step_a}")
    else:
        print(f"  [Step A] did not fire (bull EPS already ≥ 0.95×consensus_high)")
    print()
...
def layer5() -> None:
    ...
    print(f"  flags (forbidden_vocab): ({len(fv)})")
    for flag in fv:
        print(f"    • {flag}")
    print(f"  flags (citation_errors): ({len(ce)})")
    for err in ce:
        print(f"    • {err}")
    ...
    rec_label           = math.get("recommendation", "?")
    ...
    try:
        r = subprocess.run(
            ["python", "-m", "pytest", "tests_methodology.py", "-q", "--tb=no"],
            capture_output=True, text=True, cwd=Path(__file__).parent
        )
        last_line = [l for l in r.stdout.strip().splitlines() if l.strip()]
        pytest_summary = last_line[-1] if last_line else r.stdout[:120]
    except Exception as e:
        pytest_summary = f"(pytest error: {e})"

    print(f"  KO current_price:        ${current_price}")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Stage 4 layer-by-layer pipeline test (KO)")
